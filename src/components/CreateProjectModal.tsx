...
export function CreateProjectModal({ isOpen, onClose, onCreate }: CreateProjectModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white/90 dark:bg-white/10 backdrop-blur-xl border border-white/20 text-slate-800 dark:text-white w-full max-w-[600px] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Footer */}
        <div className="p-4 bg-white/40 dark:bg-black/20 flex justify-end gap-3 border-t border-slate-200 dark:border-white/10">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl font-medium text-sm text-slate-700 dark:text-gray-300 hover:bg-white/60 dark:hover:bg-white/10 transition-colors"
          >
            Cancelar
          </button>
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-blue-600 dark:bg-blue-500 text-white font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors shadow-sm"
          >
            Criar projeto
          </button>
        </div>
